...
list = ["Game 0: 0 blue, 0 red; 0 blue, 0 green, 1 red; 13 red,  11 blue",
...
def parse_games(list, red, green, blue):
    """
    Parse game descriptions and calculate the product of maximum counts for each color in each rule-compliant game.

    Parameters:
    - list (list): A list of strings representing game descriptions.
    - red (int): Maximum allowed count for the red color.
    - green (int): Maximum allowed count for the green color.
    - blue (int): Maximum allowed count for the blue color.

    Returns:
    - int: The product of maximum counts for each color in all games.

    Function Description:
    This function parses a list of game descriptions, each containing information about different colors (red, green, blue) and their counts in each round of the game. It calculates the product of the maximum counts for each color across all rounds in all rule-compliant games.
    """
    
    ans = 0
    parsable_game_list =  [x.split(": ") for x in [x.replace("Game ", "") for x in list]]
    for game in parsable_game_list:
        pass_flag = True
        parsed_game = [x.split("; ") for x in game]
        parsed_game_max_values = [0,0,0]
        for index, round in enumerate(parsed_game[1]):

            #red check
            red_match = re.search('\d+ red', round)
            if red_match:
                if int(red_match.group(0).split(" ")[0]) > parsed_game_max_values[0]:
                    parsed_game_max_values[0] = int(red_match.group(0).split(" ")[0])
                
            #green check
            green_match = re.search('\d+ green', round)
            if green_match:
                if int(green_match.group(0).split(" ")[0]) > parsed_game_max_values[1]:
                    parsed_game_max_values[1] = int(green_match.group(0).split(" ")[0])

            #blue check
            blue_match = re.search('\d+ blue', round)
            if blue_match:
                if int(blue_match.group(0).split(" ")[0]) > parsed_game_max_values[2]:
                    parsed_game_max_values[2] = int(blue_match.group(0).split(" ")[0])
        ans += np.prod(parsed_game_max_values)
    return ans

print(parse_games(list, 12, 13, 14))