...
def calc_grid(grid):
    """
    Calculate the sum of products of pairs of adjacent numeric values surrounding each asterisk in the grid.

    Parameters:
    - grid (list of str): The grid represented as a list of strings.

    Returns:
    - int: The sum of products of pairs of adjacent numeric values surrounding each asterisk.

    Function Description:
    The function iterates through each row of the grid, identifying asterisks ('*'). For each asterisk found, it determines
    the adjacent numeric values in the left, right, up, and down directions. It then calculates the product of pairs of 
    adjacent numeric values and adds them to a list. Finally, the function returns the sum of all the products.

    Note: The asterisk is assumed to have exactly two adjacent numeric values in each direction.
    """
    ans = []
    min_y = 0
    max_y = len(grid) - 1
    for y_index, y_line in enumerate(grid):
        min_x = 0
        max_x = len(y_line)

        gears = re.finditer('\*', y_line)
        for gear in gears:
            search_left = gear.start(0)
            search_right = gear.end(0) -1
            search_up = y_index
            search_down = y_index
            nearby_value = []

            # Check if left is safe
            if search_left != min_x:
                search_left -= 1   

            # Search left
            if search_left != min_x:
                for number in re.finditer('\d+', grid[y_index]):
                    if search_left <= (number.end(0) - 1) < search_right:
                        nearby_value.append(int(number.group(0)))

            # Check if right is safe
            if search_right != max_x:
                search_right += 1

            # Search right
            if search_right != max_x:
                for number in re.finditer('\d+', grid[y_index]):
                    if number.end(0) > gear.end(0) >= number.start(0):
                        nearby_value.append(int(number.group(0)))
            ...
            if search_up != 0:
                search_up -= 1
                # Search up
                for number in re.finditer('\d+', grid[search_up]):
                    if search_left <= (number.end(0) - 1) <= search_right or search_right >= number.start(0) >= search_left:
                        nearby_value.append(int(number.group(0)))
                            
            #check if down is safe
            if search_down != max_y:
                search_down += 1
                # Search down
                for number in re.finditer('\d+', grid[search_down]):
                    if search_left <= (number.end(0) - 1) <= search_right or search_right >= number.start(0) >= search_left:
                        nearby_value.append(int(number.group(0)))

            if len(nearby_value) == 2:
                ans.append(np.prod(nearby_value))
