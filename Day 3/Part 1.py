...
def calc_grid(grid):
    """
    Calculate the sum of numeric values within a grid based on adjacent special characters.

    Parameters:
    - grid (list): A list of strings representing a grid containing numeric values and special characters.

    Returns:
    - int: The sum of numeric values associated with adjacent special characters.

    Function Description:
    This function takes a grid represented as a list of strings containing numeric values and special characters. It iterates through the grid, identifies numeric values using regular expressions, and calculates the sum of values associated with adjacent special characters '@', '#', '$', '%', '&', '*', '/', '=', '-', '+'. The adjacency is determined in all eight directions (horizontal, vertical, and diagonal).
    """
    ...
    for y_index, y_line in enumerate(grid):
        ...
        matches = re.finditer('\d+', y_line)
        for match in matches:        
            search_left = match.start(0)
            search_up = y_index
            search_right = match.end(0)
            search_down = y_index + 1
            ...
            for line in grid[search_up:search_down]:
